...
private:
    ...
    {
        // Example threshold values for detecting a cylinder
        float cylinder_radius = 0.15; // 30 cm diameter
        ...
        for (size_t i = 0; i < scan->ranges.size(); ++i)
        {
            if (scan->ranges[i] < scan->range_max && scan->ranges[i] > scan->range_min)
            {
                ...
                // Simple heuristic: identify if a scan point is roughly within the cylinder radius
        ...
        {
            // Get the global position of the cylinder using TF
            try
            {
...
void drawCylinderOnMap(const std::string& original_map_path, const std::string& new_map_path, float cylinder_x, float cylinder_y)
{
    ...
    // Map origin (from map.yaml)
    float origin_x = -11.3;  // From your YAML file
    float origin_y = -5.4;   // From your YAML file
    float map_resolution = 0.1;  // 5 cm per pixel, from your YAML file
    ...
    RCLCPP_INFO(this->get_logger(), "Cylinder pixel x calculation: (%f - %f) / %f = %d", cylinder_x, origin_x, map_resolution, cylinder_pixel_x);
    RCLCPP_INFO(this->get_logger(), "Cylinder pixel y calculation: (%f - %f) / %f = %d", cylinder_y, origin_y, map_resolution, cylinder_pixel_y);
...
int map_cols = map_image.cols; // Number of columns (width)
int map_rows = map_image.rows;  // Number of rows (height)

RCLCPP_INFO(this->get_logger(), "Map dimensions: %d columns, %d rows", map_cols, map_rows);
    // Log for debugging
    ...
    int radius = static_cast<int>(0.3/ 0.05);  // Radius of 0.5m converted to pixels
    ...
    // Optional: Display the new map to verify
    ...
    void writeCylinderMapYaml(const std::string& yaml_path)
    {
        std::ofstream yaml_file(yaml_path);
        if (!yaml_file.is_open())
        {
            RCLCPP_ERROR(this->get_logger(), "Could not create YAML file: %s", yaml_path.c_str());
            return;
        }

        // Write the YAML content
        yaml_file << "image: sprint3_cylinder_map.pgm\n";
        yaml_file << "mode: trinary\n";
        yaml_file << "resolution: 0.05\n"; // Example: 5 cm per pixel
        yaml_file << "origin: [-11.3, -5.4, 0]\n"; // Assuming this is the same as your original map
        yaml_file << "negate: 0\n";
